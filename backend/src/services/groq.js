...
async function parseVendorEmail(emailContent, rfpContext) {
  ...
  const systemPrompt = `You are an AI assistant that extracts structured proposal data from vendor email responses.

The original RFP requested:
${JSON.stringify(rfpContext, null, 2)}

Extract the following from the vendor's email and return ONLY valid JSON (no markdown, no explanation):
...
Be precise with numbers. Extract exactly what is stated.`;
  ...
  const response = await groq.chat.completions.create({
    ...
    messages: [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: `Vendor email content:\n\n${emailContent}` }
    ],
    temperature: 0.2,
    max_tokens: 2000,
  });
...
async function compareProposals(rfp, proposals) {
  ...
  const systemPrompt = `You are an AI assistant that helps evaluate and compare vendor proposals for an RFP.
